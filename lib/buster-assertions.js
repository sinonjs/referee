...
(function () {
    ...
    assert.hasPrototype = function (actual, protoObj, message) {
        message = prepareAssertion("assert.hasPrototype", arguments, 2);

        if (message === null) {
            return;
        }

        if (!protoObj.isPrototypeOf(actual)) {
            return fail("assert", "hasPrototype", "msgFail", message, actual, protoObj);
        }

        ba.emit("pass", "assert.hasPrototype", message, actual, protoObj);
    };

    assert.hasPrototype.msgFail = "${0}Expected ${1} to have ${2} on its prototype chain";

    refute.hasPrototype = function (actual, protoObj, message) {
        message = prepareAssertion("refute.hasPrototype", arguments, 2);
        ...
        if (protoObj.isPrototypeOf(actual)) {
            return fail("refute", "hasPrototype", "msgFail", message, actual, protoObj);
        }

        ba.emit("pass", "refute.hasPrototype", message, actual, protoObj);
    };

    refute.hasPrototype.msgFail = "${0}Expected ${1} not to have ${2} on its prototype chain";
