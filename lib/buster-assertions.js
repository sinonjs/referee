...
(function () {
    ...
    function defineAssertion(type, name, func, fl, messageValues) {
        ba[type][name] = function () {
            ...
            if (!func.apply(ctx, arguments)) {
                return fail.apply(ctx, [type, name, "msgFail"].concat(args));
            }
    ...
    ba.add = function (name, options) {
        ...
        assert[name].msgFail = options.assertMessage;
        refute[name].msgFail = options.refuteMessage;
    ...
    function fail(type, assertion, msg) {
        ...
        var message = interpolateProperties(
            interpolatePosArg(ba[type][assertion][msg],
    ...
    assert.msgFail = "[assert] Expected ${0} to be thruthy";
    ba.count = 0;
    ...
    function multiLineStringDiff(actual, expected) {
        ...
        var message = interpolatePosArg(assert.equals.msgMultiLineStringHeading),
    ...
    assert.exception.msgTypeNoException = "Expected ${1} but no exception was thrown";
    assert.exception.msgFail = "Expected exception";
    assert.exception.msgTypeFail = "Expected ${1} but threw ${2} (${3})";
    ...
    refute.exception = function (callback) {
        ...
        if (err) {
            fail("refute", "exception", "msgFail", err.name, err.message, callback);
        } else {
