...
(function () {
    ...
    function match(object, matcher) {
        ...
        if (toString.call(object) == "[object Array]" &&
            toString.call(matcher) == "[object Array]") {
    ...
    assert.exception = function (callback, matcher, message) {
        ...
        if (!err) {
            if (typeof matcher === "object") {
                return fail.call({}, "assert", "exception", "typeNoExceptionMessage",
                                 message, ba.format(matcher));
            } else {
                return fail.call({}, "assert", "exception", "message", message);
            }
        }

        if (typeof matcher === "object" && !match(err, matcher)) {
            return fail.call({}, "assert", "exception", "typeFailMessage",
                             message, ba.format(matcher), err.name, err.message);
        }
        ...
        ba.emit("pass", "assert.exception", message, callback, matcher);
    };
    ...
    assert.exception.matchFailMessage = "${0}Expected thrown ${1} (${2}) to pass matcher function";
