...
function refute(actual, message) {
    referee.countAssertion();
    if (!assertArgNum(referee.fail, "refute", arguments, 1)) { return; }
...
function multiLineStringDiff(actual, expected, message) {
    ...
    var failureText = interpolateProperties(referee, heading, { customMessage: message });
...
referee.assert.equals.multiLineStringHeading = "${customMessage}Expected multi-line strings " +
    "to be equal:\n";
...
referee.assert.match.exceptionMessage = refute.match.exceptionMessage = "${customMessage}${exceptionMessage}";
