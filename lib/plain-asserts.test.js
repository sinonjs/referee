...
describe("plain asserts", function() {
        ...
        return referee.assert.equals
            ...
            .then(undefined, function(message) {
                referee.assert.equals(
                    message,
                    "[assert.equals] actual expected to be equal to expected"
                );
            });
    });
    it("proper custom failure message without args", function() {
        return referee.assert.equals
            .test("actual", "expected", "custom message")
            .then(undefined, function(message) {
                referee.assert.equals(
                    message,
                    "[assert.equals] custom message: actual expected to be equal to expected"
                );
            });
    });
    ...
    it("this.fail works from assertions", function() {
        return referee.assert.className
            .test({}, "Item")
            .then(undefined, function(message) {
                referee.assert.equals(
                    message,
