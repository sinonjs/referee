...
(function () {
    ...
    assert.exception = function (callback, exception, message) {
        ...
        if (exception && err.name != exception) {
            return fail("exception", "msgTypeFail", message, exception, err.name, err.message);
        }
    ...
    assert.noException = function (callback, message) {
        ...
        if (err) {
            fail("noException", "msgFail", message, err.name, err.message, callback);
        } else {
            assert.emit("pass", "noException", message, callback);
        }
    };

    assert.noException.msgFail = "${0}Expected not to throw but threw ${1} (${2})";
    ...
    if (typeof module != "undefined") {
        ...
        module.exports.that = function () {
            module.exports.that = require("./buster-assert/assert-that");
            return module.exports.that.apply(module.exports, arguments);
        }
    }
}());
